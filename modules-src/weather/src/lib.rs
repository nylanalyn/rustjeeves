...
use jeeves_abi::{
    ...
    StatIncrement, ThemeReq, WeatherAlert, WeatherAlertsResult, WeatherQuery, WeatherResult,
...
pub fn data_delete(input: String) -> FnResult<String> {
    ...
    let keys = [
        ...
        retired_local_cooldown_key(&request.subject.server, &request.subject.profile_id),
    ];
